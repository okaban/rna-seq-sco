...
**Date:** 2026-02-27
**Hypothesis:** H31 - The 62 "exposed" transcription factors regulate a substantial downstream gene network, amplifying the methylation signal from 62 regulators to hundreds of target genes
**Status:** PARTIAL
...
- **993 "shielded" regulators**: possess methylation protection zones (median nearest methylation distance = 762 bp from TSS)
- **62 "exposed" regulators**: lack protection zones (median nearest methylation = 114 bp from TSS, 6.7x closer)

Only the 62 exposed regulators show methylation-expression coordination (H27). All 62 are dynamically expressed (100% DEG rate, H28), suggesting they serve as methylation-responsive signal transducers.

**The cascade amplification hypothesis**: If each exposed TF regulates ~50-100 target genes (typical for bacterial TFs), the 62 exposed regulators could relay the methylation signal to 3,000-6,000 downstream genes -- a massive amplification of the epigenetic signal.
...
None of the 62 exposed regulators have curated binding motifs in the FIMO database. The 16 TFs with FIMO motifs (SigB, AbrC3, SigR, SigE, NdgR, AfsQ1, GlnR, PhoP, BldD, AdpA, ArgR, Crp, DasR, DraR, HrdB, ScbR) are all well-characterized regulators from RegPrecise/literature. This means:

- **Direct regulon mapping of the 62 exposed TFs is not possible** with current FIMO data
...
|---|---|---|---|---|
| Targeted by FIMO TFs | 38/62 (61.3%) | 638/993 (64.2%) | 0.881 | 0.683 |
...
The fundamental limitation is that the 62 exposed regulators are **newly identified, uncharacterized regulatory genes**. They include:
...
1. **38/62 (61.3%) exposed regulators are under control of well-characterized TFs** (SigB, AbrC3, SigR primarily). This places them within the characterized regulatory hierarchy.
...
Rather than the original hypothesis of 62 exposed TFs -> N target genes (direct cascade), the data supports a **parallel pathway model**:

- **Pathway A (characterized)**: SigB/SigR/AbrC3 -> ~5,400 target genes (broad, non-specific)
- **Pathway B (methylation-responsive)**: Methylation -> 62 exposed TFs -> unknown targets
...
1. **SUPPORTED**: 38/62 exposed TFs are connected to the characterized regulatory network as targets of SigB/AbrC3/SigR
2. **NOT TESTABLE**: Direct regulon sizes of the 62 exposed TFs cannot be determined without binding motif data
...
4. **SUPPORTED (indirect)**: The 62 exposed TFs, being 100% dynamically expressed (H28), likely regulate substantial downstream gene sets based on typical bacterial TF regulon sizes (10-100 genes each)

**Estimated cascade (indirect)**: If each exposed TF regulates ~30 genes (conservative estimate for Streptomyces), the 62 exposed TFs could control ~1,860 unique genes (~22% of genome), but this is purely hypothetical without motif data.
...
|---|---|
| **H27** | Identified the 62 exposed regulators as methylation-responsive (TSS methylation 6.7x closer, protection zone absent) |
| **H28** | All 62 exposed regulators are dynamically expressed (100% DEG, vs ~50% for shielded); H31 shows this is not due to upstream FIMO TF regulation |
...
| **H25** | Protection zone model: 993 regulators have -1300/+700bp protection; H31 shows the 38 exposed TFs within FIMO TF reach do not gain protection benefit |
...
| Exposed regulators as FIMO TF targets | 38 of 62 (61.3%) |
| Shielded regulators as FIMO TF targets | 638 of 993 (64.2%) |
