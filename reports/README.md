...
| `260224_H6_genomewide_TF_screen_report.md` | 2026-02-24 | **H6: ゲノムワイド制御因子メチル化-発現スクリーニング** - GFFから1,055制御遺伝子抽出(25ファミリー)。165/1,055 (15.6%)がメチル化保有（文献37の6.7%の2.3倍）。**57個で協調変動検出**（全て37リスト外）。SC_RS24635がT2+T3 concordant repression。MerR最高メチル化率28.6%、SARP完全非メチル化 |
...
| `260224_H11_gatekeeper_model_v2_report.md` | 2026-02-24 | **H11: Gatekeeper Model v2定量的統合** - H1-H9全結果を3層モデルに統合。Layer 1: Landscape Remodeling (4mC rho=0.139, Jaccard=0.000)。Layer 2: Protection/Depletion (TF BS fold=0.66, sigma-10 p=1.73e-12)。Layer 3: Signal Gating Revised (57非文献制御因子, TCS非対称, COG T OR=2.47)。**Excluded**: TFカスケード経路(0/37協調変動)。10個のTestable Predictions |
...
| `260226_H27_coordinated_regulators_protection_report.md` | 2026-02-26 | **H27: 57協調変動制御遺伝子のProtection Zone特性解析** - H25のprotection zoneが協調変動57遺伝子と非協調998遺伝子で異なるか検証。最近接メチル化サイト距離: 協調=114bp vs 非協調=762bp(6.7倍近接, p=5.3e-28)。協調遺伝子はprotection zone**完全欠如**(幅0bp, 深さ0.000)、代わりにTSS直上で**8.4倍エンリッチメント**(密度3.15 vs 0.37 sites/kb/gene)。Delta-LFC相関は協調群のみ有意(rho=0.277, p=0.029)。地理分布は非有意(OR=1.26, p=0.42)。4協調型全てで保護欠如。**SUPPORTED(逆方向)**: "Exposed Promoter"モデル - 998遺伝子はプロモーター保護で発現安定、57遺伝子はメチル化透過で発現応答 |
| `260226_H28_exposed_regulators_characteristics_report.md` | 2026-02-26 | **H28: 57 Exposed制御遺伝子の生物学的特性解析** - 57 exposed vs 998 shielded制御遺伝子の包括的多変量比較(19検定)。T1発現有意に低い(median 90 vs 123, p=0.010, r=0.195)。**全57遺伝子が動的発現(constitutive=0%, OR=inf, p=5.3e-13)**。|log2FC|が1.7倍大(T3: p=1.7e-09, r=0.456)。プロモーターGC含量高い(71.5% vs 69.7%, p=2.0e-04)。TFファミリー・地理分布・オペロン構造は非有意。T1最低四分位にexposed 45.2%集中(3.1倍enrichment)。**SUPPORTED**: 低初期発現+100%動的制御+高GCプロモーターの3因子シナジーモデル |
...
| `260227_H31_exposed_TF_downstream_network_report.md` | 2026-02-27 | **H31: Exposed TF下流制御ターゲットネットワーク解析** - 57 exposed TFはFIMOモチーフ非保有（レギュロン直接測定不可）。16 FIMO TFから9,635 BS(q<0.05)、5,430標的遺伝子。38/57 exposed TFがFIMO TFの標的（61.3%、shielded 64.2%と非有意差 OR=0.881, p=0.683）。標的 vs 非標的の発現差なし（\|LFC\| p=0.25/0.67）。FIMO TF間10本のcross-regulation edge（GlnRがhub）。機能エンリッチメントなし。**PARTIAL**: exposed TFは特徴付けられた制御ネットワーク内に存在するが、メチル化応答は上流TF制御とは独立 |
| `260227_H32_exposed_regulatory_module_report.md` | 2026-02-27 | **H32: 57 Exposed制御遺伝子の自己制御モジュール解析** - ゲノム上クラスタリング非有意(z=0.30, p=0.37)。E-E共発現はE-S/S-Sより高くない(p=0.26/0.99)。**協調型内共発現は極めて強い**(within rho=0.500 vs between=-0.283, p=6.6e-25, r=0.68)。4共発現モジュール(61/57遺伝子カバー): 活性化ブロック(35遺伝子)と抑制ブロック(26遺伝子)の二分法。TCS 7ペア全て非対称(exposed:shielded=1:1)、SK/RRバイアスなし(4:3, p=1.0)。オペロンペア0。**PARTIAL**: 物理的モジュールではなく「分散型メチル化応答制御層」。2つの拮抗プログラムがcoordination typeで構造化 |
| `260227_H33_neighborhood_transcriptional_impact_report.md` | 2026-02-27 | **H33: Exposed TFゲノム近傍転写影響解析** - 57 exposed TFの±10/20/50kb近傍遺伝子の発現変動検証。近傍\|LFC\|はshielded TFと差なし(MWU p=0.94)。方向一致率0.599(shielded 0.556, p=0.191)。1,000並べ替えでZ=-1.03, p=0.857。距離減衰効果なし(rho=+0.026, NS)。活性化/抑制ブロック近傍方向差は有意(chi2 p=9.2e-19)だが地理的交絡の可能性。BGC pathway-specific regulator 0件。**REJECTED**(主仮説): exposed TFは古典的cis局所制御因子ではなく、遺伝子自律的メチル化応答。**PARTIAL**: ブロック方向性分岐 |
| `260227_H34_temporal_dynamics_exposed_TF_report.md` | 2026-02-27 | **H34: 57 Exposed TFの時間的ダイナミクスと階層構造解析** - 39/57(63%)がearly responder(T1→T2で主要変動)。活性化ブロックと抑制ブロックは**同期的応答**(位相分離なし, MW p=0.459)。4協調型間のphase ratio差なし(KW p=0.638)。メチル化タイミングと発現タイミングは無相関(rho=0.136, NS)。TCS 7ペアで系統的exposed-first順序なし(Wilcoxon p=0.813)。モジュール内temporal coherence強い(rho=0.717, p=1e-19)がモジュール間予測力ゼロ。Exposed vs shielded temporal差なし(MW p=0.180)。**PARTIAL**: 同時スイッチモデル(カスケードではなく並列起動) |
| `260227_H35_AAGCCCG_exposed_TF_causal_report.md` | 2026-02-27 | **H35: AAGCCCG Exposed TF因果経路解析** - SC_RS17645→AAGCCCG 6mA→57 exposed TFの因果経路検証。**致命的発見: H30の配列モチーフ5倍enrichment(25.8%)はメチル化enrichmentに翻訳されず、わずか2/57(3.2%)のみがTSS±500bpにメチル化AAGCCCG保有**。Lost vs Never: p=0.402, r=0.383(予測と逆方向)。用量反応NS(rho=-0.117)。SC_RS17645相関差NS(p=0.346)。Bootstrap CI全て0を含む。**NOT SUPPORTED (UNTESTABLE)**: メチル化AAGCCCG部位数が少なすぎ、配列エンリッチメントはメチル化レベルに反映されない。57 exposed TFの制御機構はAAGCCCGメチル化以外 |
| `260227_H35_TF_family_functional_prediction_report.md` | 2026-02-27 | **H35: 57 Exposed TFのTFファミリー基盤機能予測解析** - 16 TFファミリーの機能アノテーション、ブロック間組成比較、ゲノム近傍文脈解析。**TetRが抑制ブロックに有意集積**(OR=0.16, p=0.028)。活性化ブロック: TCS 9 (25%, OR=4.0)、sigma因子5、WhiB 1 → 形態分化・シグナル伝達。抑制ブロック: TetR 7 (efflux隣接)、代謝TF (GntR/IclR/LacI/LysR)、DNA複製/修復 (SSB/HU/Mfd/UdgX) → 栄養増殖プログラム停止。**SUPPORTED (qualitative model)**: 2ブロック系は栄養増殖→発達転換スイッチ |
| `260227_H36_exposed_TF_conservation_report.md` | 2026-02-27 | **H36: 57 Exposed TF進化的保存性解析** - 配列組成(GC3, Nc, rare codon)、アノテーション品質(gene_name, SCO tag, hypothetical)、染色体位置(core/arm, oriC距離)、発現レベル、複合スコアで比較。**複合保存スコアに有意差なし**(p=0.276, d=0.141, ROC AUC=0.459)。17検定中有意4件はいずれも発現動態(\|LFC\| p<1e-5, constitutive OR=0.000)。SCO tag 96.8%、named product 100%。**PARTIALLY SUPPORTED**: exposed TFは配列的に同等に保存されており、系統特異的新規遺伝子ではなく古代の保存特徴。exposed/shieldedの区別はエピゲノム文脈によるもの |
| `260227_H37_coexpression_regulon_prediction_report.md` | 2026-02-27 | **H37: ゲノムワイド共発現レギュロン予測解析** - 57 exposed TFの活性化/抑制ブロック固有遺伝子(eigengene)に対するSpearman相関。活性化レギュロン28遺伝子(0.4%)、抑制レギュロン131遺伝子(1.7%)。**Jaccard=0.000の完全相互排他**。Cross-eigengene rho=-0.995。抑制ブロックは置換検定有意(Z=5.42, p=0.003)、活性化ブロックは非有意(p=0.123)。synthase 4.86倍enriched(FDR=4.3e-05)。DEG方向一致率100%。Per-TF median regulon: act=11, rep=14。**SUPPORTED (with caveats)**: 2プログラムモデル支持だが、rho=-0.995は単一発達軸の表裏。n=9の検出力限界 |

### 統合モデル（2026-02-27）

| ファイル | 日付 | 内容 |
|---------|------|------|
| `260227_gatekeeper_model_v3_synthesis_report.md` | 2026-02-27 | **Gatekeeper Model v3 包括的統合レポート** - 14探索ループ・29仮説の最終統合。中心発見: 1,055制御遺伝子のShielded/Exposed二分法（998 shielded vs 57 exposed、293bp境界、AUC=0.917）。4層モデル: Layer 1=R-M防御地理的再配置（転写制御なし）、Layer 2=制御DNA保護（25%配列進化+75%集合的プロモーター占有）、Layer 3=57 exposed遺伝子のメチル化応答カスケード。Simpson's paradox同定（H17/H19）、時間的脱抑制否定（H19/H21）、発現非依存保護（H29）等の重要な否定的結果を統合 |
| `260227_gatekeeper_model_v4_final_synthesis_report.md` | 2026-02-27 | **Gatekeeper Model v4 最終統合レポート** - 18探索ループ・36仮説の完全統合。v3(H1-H29)にH30-H36を追加。新知見: 配列寄与~33%(H30)、0/57 FIMOモチーフ・並列経路モデル(H31)、分散型制御層・活性化/抑制2ブロック(H32)、近傍効果なし・trans作用(H33)、同時スイッチ(H34)、AAGCCCG配列≠メチル化enrichment(H35-causal)、TetR抑制ブロック集積・栄養増殖→発達転換スイッチ(H35-TF)、同等保存性・エピゲノム文脈(H36)。Verdict分布: 7 supported, 15 partial, 12 rejected |
### 論文準備資料（2026-02-24）
...
| `260420_A7_exposed_TF_heatmap_report.md` | 2026-04-20 | **A-7: Exposed制御遺伝子 発現Heatmap (Figure 5b)** - 57遺伝子×9サンプル(T1/T2/T3×3)。Ward k=3クラスター: C3(n=25)がT2 DEGを全包含（23/23）。T2 up-regulated 23/57 (40.4%)。Gatekeeper Model v4 Layer 3「T2協調的upregulation」を直接可視化 |
| `260504_paper_tone_down_proposal_report.md` | 2026-05-04 | **論文トーンダウン文案** - Exposed TF (n=57) のTSS近傍メチル化変化 vs T1→T2 LFC散布図解析を踏まえた書き換え提案。Spearman ρ=0.13–0.30（窓 ±200–1000bp）、GCCGGC+AAGCCCG限定では NS、符号は「保護崩壊→脱抑制」予測と逆。Abstract / Results R3-R4 / Discussion limitation の具体文案を提示 |
| `260504_methylation_correlation_sensitivity_allangles_report.md` | 2026-05-04 | **メチル化–発現相関 多角的感度解析** - n=57 exposed TFs。6 angle × 3時点ペア × 5モチーフ × 6窓サイズ。**Δmethylation × LFC は全 angle で chance level**（角1/2/4/5a hits ≤7.3%）。一方 **count_T1（絶対メチル化数）× LFC は 2.6–3.1× chance**（all_4mC ±293bp で ρ=−0.40, p=0.002）。core/arm partial corr 後も最強セルが ρ=−0.33, p=0.012 で残存。結論：「Δ–Δ 相関なし」は維持、「メチル化と発現は無関係」は否定。T1 4mC density が T1→T2 抑制を予測する状態依存的シグナル |
| `260504_v_defense_4mC_hypo_GCCGGC_report.md` | 2026-05-04 | **COG V-Defense × 4mC-hypo 検証** - 12遺伝子（V-Defense ∩ 4mC-hypo, |Δ4mC|>10%）の構成と GCCGGC プロモーター配置。**10/12 が SAM-MTase**、Pgl/BREX (SCO5331/6626-6636) と methyl-specific restriction nuclease (SCO4213/4631/2863) はいずれも 4mC change=0 で集合に含まれず。GCCGGC は −200/+50 プロモーターで 3/12 (25%) のみ陽性、しかも 3 件全てが GFF-fallback TSS。「promoter-GCCGGC 脱メチル化 → 脱抑制」モデルは ≥75% で不成立、責任サイトは遺伝子本体側に存在 |
| `260504_v_defense_MTase_expression_dynamics_report.md` | 2026-05-04 | **V-Defense MTase 発現動態 vs 全ゲノム GCCGGC-m4C 密度** - 12遺伝子（V-Defense ∩ 4mC-hypo, 10/12 SAM-MTase）の T1/T2/T3 発現軌跡と GCCGGC-m4C 密度（T1=4.10%→T2=1.29%→T3=0.07%, ≈60×崩壊）の対比。**自己調節ループ仮説は 12 遺伝子平均では成立せず**（T1→T2 down 5/12, up 7/12; mean log2 は T2 でピーク 7.89）。Pearson r=−0.27 (n=3, NS)。**4 単調減少サブセットに限ると Pearson r=+0.975**。DNA-MTase 候補は **SC_RS12890 (SCO2170, class I SAM-MTase)** および SC_RS38275/SC_RS38280 (SCO7213) の局所クラスタのみ。"set-level coordinated MTase shutdown" は Simpson 様集約アーチファクト |
| `260505_exposed_4mC_6mA_split_correlation_report.md` | 2026-05-05 | **Exposed TF (n=57) を 4mC選択群 (n=19) / 6mA選択群 (n=37) / 両方 (n=1) に分割した T1 メチル化密度 × T1→T2 LFC 相関** - プールされた ρ=−0.40 (p=0.0019) は **4mC選択群** が主に駆動。4mC選択群 ρ_4mC=−0.33 (p=0.17), ρ_6mA=−0.37 (p=0.12)（n=19, 方向一致だが underpowered）。6mA選択群 ρ_4mC=−0.17 (p=0.32), ρ_6mA=−0.09 (p=0.61)（信号消失）。「絶対 T1 4mC density が T1→T2 抑制を予測」シグナルは 4mC 動態を持つ遺伝子に局在。出力: `Fig_exposed_4mC_vs_6mA_split_correlation.{pdf,png}`, `exposed_4mC_6mA_split_correlation.tsv`, Obsidian: `Writing/Fig_exposed_4mC_6mA_split.png` |

### Figure生成（2026-05-06）

| ファイル | 日付 | 内容 |
|---------|------|------|
| `Writing/motif_methylation_position_figure.pdf` | 2026-05-06 | **モチーフ位置別メチル化確率Figure** — Panel A: GCCGGC m4C（pos2=+strand 3.61%, pos3=−strand 3.22%、n=15,726 occurrences）、Panel B: AAGCCCG 6mA（pos2=7.58%一次, pos1=2.07%二次、n=1,334）。T1タイムポイント。スクリプト: `Writing/make_motif_position_figure.py` |

### 文献調査・論文準備（2026-05-06）

| ファイル | 日付 | 内容 |
|---------|------|------|
| `260506_m4C_vs_m5C_citation_report.md` | 2026-05-06 | **m4C vs m5C at GCCGGC 論拠支持文献レポート** - 「Pisciotta 2023のbisulfite-seqによるm5C帰属は誤り、正しくはm4C」論の科学的根拠となる文献13件を4カテゴリで整理（bisulfite限界、Nanopore m4C検出、細菌R-Mシステム、Streptomycesメチル化・分化） |
| `260506_HiC_FIRE_HupS_HupA_integration_report.md` | 2026-05-06 | **Hi-C FIRE × Protection Zone & HupS/HupA発現統合** - Deng et al. 2023 (PNAS) Hi-C論文との統合解析。**FIRE overlap**: FIREデータなし（ネットワーク制限）、将来方向として bedtools パイプライン準備済み（Shielded=998, Exposed=57 TSS座標あり）。**HupS/HupA発現**: HupS (SC_RS07825/SCO1175) はT1→T3で+3.70 log2FC (padj=1.1×10⁻⁴) の強い散布期特異的誘導。HU-family constitutive (SC_RS16855/SCO2950, HupA候補) はT3/T1比 −2.90 log2FC (padj=2.9×10⁻³⁴) で単調減少。⚠ SCO4950は現行RefSeqではnarI (硝酸還元酵素) にマップ、HupAとの同定には要確認。NAP handoverはT1→T2遷移で発生し保護ゾーン溶解・m4C再分布と時間的一致。 |
| `260506_epigenome_pipeline_QC_report.md` | 2026-05-06 | **エピゲノムパイプライン包括QCレポート** — 9段階QCチェック（カバレッジ、アライメント、レプリケート一致性、修飾モデル・閾値、サイト数・モチーフ、既知外れ値、RNA-seq統合、整合性スクリプト、エラー/警告）。主要PASS: RNA-seq 98.4–98.7%、Nanopore 85–92% アライメント率、全12,429サイト n_reps=3。主要WARN: AL645882.2 vs NC_003888.3 参照ゲノム表記不一致、6mAベースコーリングモデル未文書化、pileupディレクトリパス不整合（cursor_results vs analysis/pileup）、T3平均カバレッジ低め(60.7x)、旧manuscriptのサンプル名 (2-2/3-1) 要更新。FAIL: check_integrity.py 不存在、T1 rep1「1,249サイト」問題の確認不可。投稿前優先アクション8件を提示。 |
| `260506_HupA_locus_verification_report.md` | 2026-05-06 | **HupA遺伝子座検証レポート** — Salerno 2009 (PMID 19717607) および現行RefSeq NC_003888.3 に基づくHupA正規遺伝子座の確認。**結論: SCO2950 (SC_RS16855) = HupA**（HU family DNA-binding protein、T1 mean 22,368 normalized counts → T3 2,932、log₂FC −2.90）。SCO4950 は narI（硝酸還元酵素γサブユニット）であり HupA ではない — 論文 Discussion の訂正必須。HupS 遺伝子座（SCO5556 vs SCO1175）についても注意喚起あり（要タンパク質配列による確認）。 |
| `260506_20gene_sequence_structure_similarity_report.md` | 2026-05-06 | **カテゴリーA 20遺伝子 配列・構造類似性解析サマリー** — 既存結果（`analysis/57_temporal_dynamics_exposed_TF/`）の集約。Pfam (hmmscan E≤1e-3) + BLASTP vs 文献6リファレンス（MarR/OxyR/Lrp/HP1021/SigB/RamR）。**10/20 が文献ファミリー Pfam レベル一致**（TetR/AcrR×4, TCS-RR GerE×3, MarR×1, LysR/OxyR×1, Sigma-70 ECF×1, Lrp/AsnC×0）、3/20 が BLAST E<1e-3、pident≥30% は SCO0089 (LysR vs OxyR pident=32.0%) のみ。**HTH_24 / Sigma70_r4_2 を除外しても判定は不変**（top_pfam が独立にファミリーを支持）。オーソロジー主張は不可（Streptomyces TF の配列分岐が大きい）。新規解析なし、レポートのみ。 |
| `260510_TSS_downstream_GCCGGC_protection_report.md` | 2026-05-10 | **TSS上流・下流GCCGGC保護ゾーン解析 (analysis/75)** — Shielded (n=998) / Exposed (n=57) 制御遺伝子を対象に±1 kb TSS中心化GCCGGC 4mCメタプロファイルを構築。主要所見: GCCGGC保護ゾーンはTSS上流（0.886× vs ゲノム背景）だけでなく下流+293 bpまでも実在（0.649×）し、上流より下流の方がさらに低密度。Exposed遺伝子は上流でメチル化が集積（1.405×）し下流で中程度（0.780×）。Shielded/Exposed群間差は統計的有意でなし（サイト数1,299/1,055遺伝子でゼロ膨張）。**結論：保護は「上流優位」ではなく転写開始点±293 bpを囲む対称的低密度領域として存在し、「転写バブル形成領域全体の保護」解釈を支持。** 2パネル図: `SuppFig_protection_downstream.png` |
| `260507_layer2_layer3_overlap_report.md` | 2026-05-07 | **Layer 2 (counter-selection) × Layer 3 (NAP occupancy) overlap — Reviewer Minor-3-5 対応** — 762 shielded regulators (`nearest_methyl_dist > 293 bp`) を 2-kb extended と TSS ±600 bp protection-zone で per-gene 分解。Layer 2 deficit = expected − observed motifs; Layer 3 deficit = observed − methylated_T1。**結論: 両層は largely independent な遺伝子集団に作用**（extended Jaccard=0.097, OR=0.094, p=1.2×10⁻¹⁹; protection-zone Jaccard=0.000, OR=0, p=6.3×10⁻²⁶）。protection-zone では完全に相互排他（676 L2-only / 25 L3-only / 0 both）。Layer 2 は GCCGGC を持たない遺伝子、Layer 3 は GCCGGC を持つが unmethylated な遺伝子に対応し、site-level でも構造的に disjoint。反論: 同一現象の二重カウントではなく、独立メカニズム。 |
...
| ファイル | 日付 | 内容 |
|---------|------|------|
| `260508_peer_review_report.md` | 2026-05-08 | **模擬査読レポート（*Nature Microbiology* 水準）** — full_manuscript.md 全文・統計検定登録・STREME 結果を精読した上での包括的査読。主要懸念 6 件（MC-1: 因果言語と temporal decoupling の矛盾、MC-2: TCS asymmetry p=0.83 の過大評価、MC-3: 33/67% 計算誤り、MC-4: 実験的検証なし、MC-5: BGC 内部比較の地理的層別化欠如、MC-6: 実験的 TSS データの限界）、軽微懸念 10 件を含む。総合推薦: **Major Revision** |
| `260508_revision_log_R1.md` | 2026-05-08 | **査読対応 R1 ログ** — 260508_peer_review_report の MC-1〜MC-6 + 主な mn-* を full_manuscript.md / 01_results.md / 03_discussion.md へ反映した記録。P1 (因果言語→共在フレーム)、P2 (TCS p=0.83 明記)、P3 (33/67% 定性化)、P4 (BGC 層別化 caveat)、P5 (TSS proxy 注記)、P6 (knockout なしを明記)、P7 (5.9%→5.4%、61→56 of 57、Gatekeeper→methylation-exclusion model) を完了。次回検討項目として mn-3, mn-4, Pisciotta 2023 引用追加を保留。 |

---

*最終更新: 2026-05-10 (260510_TSS_downstream_GCCGGC_protection 追加)*
| 260420_A2_boundary_sensitivity_report.md | A-2 boundary sweep + T2/T3 independent models | 2026-04-20 |
