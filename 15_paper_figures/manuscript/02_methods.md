...
## AAGCCCG co-modification analysis

Co-modification was defined as the simultaneous presence of a high-confidence m4C call and a high-confidence 6mA call within the same AAGCCCG motif instance (intra-motif pairs at the A₀↔C₃ and A₁↔C₅ spacings, T1 pooled across replicates). At the site level, 244 co-modified AAGCCCG instances were observed against a background of 1,090 AAGCCCG sites carrying only one modification type and 2 co-modified sites in non-AAGCCCG genomic context (out of 1,236,881 non-AAGCCCG sites total). To assess whether this co-occurrence is non-random, the expected number of co-modified pairs in AAGCCCG sites under the null was computed by permutation: 10,000 hypergeometric draws of 1,334 "AAGCCCG" sites from a pool of 1,238,215 sites containing 246 co-modified sites (NumPy `np.random.hypergeometric`, seed = 42). Under this null, EXPECTED_N = 0.268 ± 0.517 (permutation mean ± SD; analytic mean = 0.265), giving a fold-enrichment of approximately 921× over the null expectation and an empirical *p* < 1×10⁻⁴ (0 of 10,000 permutations ≥ observed 244). The corresponding Fisher's exact test (one-sided) and the conditional log-OR 95% confidence interval were computed in `statsmodels` (`Table2x2`) and yielded OR = 145,448 (95% CI: 36,131–585,503).
...
TSS positions were assigned by integrating two data sources. The primary source was the gene start coordinate from the NCBI RefSeq GFF annotation. Where available, experimentally determined TSS positions from dRNA-seq data (Jeong et al. 2016) were used instead (257 genes with experimental TSS data). For genes on the minus strand, distance calculations were adjusted so that negative values indicate upstream positions and positive values indicate positions within the gene body. The final comprehensive TSS table covered 7,996 protein-coding genes.
...
TF family annotations were assigned based on protein domain architecture and product description. For each TF family represented among the 57 exposed regulators, Fisher's exact test was used to assess enrichment in the activation versus repression bloc. The TetR family was the only family achieving statistical significance after considering multiple comparisons.
