#!/usr/bin/env python3
"""Main Figure v3: T1 4mC density at exposed-TF promoters predicts T1->T2 LFC.

Differences vs. v2:
  * Category A orange edge ring removed entirely.
  * Category A entry removed from legend.
  * Gray (count_T1==0) / blue (count_T1>=1) two-group coloring kept.
  * Gene labels (ramR, SCO4122, SCO7314, tcrA) kept.

Outputs:
  analysis/figures/T1_4mC_vs_T1T2_LFC_main_v3.pdf
  analysis/figures/T1_4mC_vs_T1T2_LFC_main_v3.png  (300 dpi)
  /Users/okaban/obsidian/Research/rna-seq/Writing/T1_4mC_vs_T1T2_LFC_main_v3.png
"""
...
# Scatter: single uniform style per group, no Category A ring
ax.scatter(df.loc[zero_mask, "density_plot"], df.loc[zero_mask, "LFC_T2vsT1"],
           s=30, facecolor=COLOR_ZERO, edgecolor="black",
           linewidth=0.4, alpha=0.9, zorder=3,
           label=f"count$_{{T1}}$ = 0  (n={n_zero})")

ax.scatter(df.loc[nonzero_mask, "density_plot"], df.loc[nonzero_mask, "LFC_T2vsT1"],
           s=30, facecolor=COLOR_NONZERO, edgecolor="black",
           linewidth=0.4, alpha=0.9, zorder=3,
...
# Legend: drop Category A entry entirely
handles = [
...
# Save
# ------------------------------------------------------------------ #
PDF = FIG_DIR / "T1_4mC_vs_T1T2_LFC_main_v3.pdf"
PNG = FIG_DIR / "T1_4mC_vs_T1T2_LFC_main_v3.png"
fig.savefig(PDF)
fig.savefig(PNG, dpi=300)
plt.close(fig)

print(f"[write] {PDF}")
print(f"[write] {PNG}  (300 dpi)")

if OBSIDIAN_DIR.exists():
    OBS_PNG = OBSIDIAN_DIR / "T1_4mC_vs_T1T2_LFC_main_v3.png"
    shutil.copy2(PNG, OBS_PNG)
    print(f"[copy ] {OBS_PNG}")
else:
    print(f"[skip ] Obsidian directory not found: {OBSIDIAN_DIR}")
