...
CAVEATS = [
 dict(path="11_epigenome_integration/analysis/07_motif_analysis/methylation_site_sequences.csv",
      note="**この表は first-appearance 表である（2026-09-21 判明、BLOCKER-0）**: position で重複除去され、"
           "各部位は最初に現れた時点の行しか持たない（4mC 1,987 / 664 / 42）。**T1 の集計にしか使えない。**"
           "T2/T3 の部位数・占有率・ペア数・Jaccard をここから出すと「T2 で新たに現れた部位」を数えることになる"
           "（本文の 1,289→407→21、Jaccard 0、Exposed 62→0、27.4→6.8→3.7% の原因）。"
           "時点別の値は `01_integration/high_confidence_sites_weighted.csv`（全時点・全部位）から出すこと"
           "（`90_per_timepoint_census_audit/`）。さらに `sequence` 列の窓は真の部位より 1 塩基上流に中心がある"
           "（C₅/C₃ ラベルずれの原因）。オフセットは参照配列に position を当てて求めること。"),
 dict(path="11_epigenome_integration/analysis/23_expanded_motif_search/4mC_motif_assignment.csv",
      note="**first-appearance 表（07_ と同じ重複除去）。T2/T3 の集計に使わない。** 詳細は 07_ の READ_BEFORE_USE.md。"),
 dict(path="11_epigenome_integration/analysis/88_occupancy_series_and_CE",
      note="**T2/T3 の占有率（6.8% / 3.7%）は first-appearance 表由来で誤り**（正準: 29.0% / 28.1%、`90_`）。"
