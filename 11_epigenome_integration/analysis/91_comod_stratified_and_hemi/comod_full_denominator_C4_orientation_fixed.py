#!/usr/bin/env python3
"""ORIENTATION FIX 2026-09-21 (EPI-03, verified by the main agent on 300 reverse reads: modified_bases positions land on the
complement base in SEQ orientation 100%/ref 99.3%; modified_bases_forward only 28% = chance). The 79_ script paired
modified_bases_forward (original-read coordinates) with get_aligned_pairs (SEQ coordinates), so every minus-strand
instance scored unmodified for both marks and the pooled OR (4.84) was a Simpson artefact. Same script otherwise.
...
for bam in BAMS:
    with pysam.AlignmentFile(bam) as fh:
        for r in fh.fetch(CHROM):
            ...
            probA, probC = {}, {}
            for (canon, st, code), calls in (r.modified_bases or {}).items():
...
with open("comod_full_denominator_T1_C4_orientation_fixed.tsv","w",newline="") as fh:
